...
protocol GameModelProtocol : class {
    ...
    func moveOneTile(from: ((Int, Int), (Int, Int)), to: (Int, Int),value: Int)
...
class GameModel: NSObject {
    ...
    func timerFired(timer: Timer) {
        ...
        if changed {
            self.timer = Timer.init(timeInterval: queueDelay, target: self, selector: Selector("timerFired:"), userInfo: nil, repeats: false)
        }
    }
    ...
    func userHasLost() -> Bool {
        ...
        for i in 0..<dimension {
            for j in 0..<dimension {
                switch gameboard[i, j] {
                case .Empty:
                    assert(false, "Gameboard full, but a tile is still empty.")
                case let .Tile(v):
    ...
    func userIsWinner() -> (Bool, (Int, Int)?) {
        for i in 0..<dimension {
            for j in 0..<dimension {
                switch gameboard[i, j] {
    ...
    func performMove(direction: MoveDirection) -> Bool {
        let coordinateGenerator: (Int) -> [(Int, Int)] = { (iteration: Int) -> [(Int, Int)] in var buffer = Array<(Int, Int)>(repeating: (0, 0), count: self.dimension)
    ...
    var atLeastOneMove = false
        ...
        func condense(group: [TileObject]) -> [ActionToken] {
            ...
            for (idx, tile) in EnumeratedIterator(group) {
                switch tile {
        ...
        func collapse(group: [ActionToken]) -> [ActionToken] {
            ...
            var tokenBuffer = [ActionToken]()
            var skipNext = false
            for (idx, token) in EnumeratedIterator(group) {
                if skipNext {
                    skipNext = false
                    continue
                }
                ...
                case let t where (idx < group.count-1 && t.getValuea() == group[idx+1].getValue()):
                    let next = group[idx+1]
